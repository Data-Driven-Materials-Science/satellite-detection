...
for cycle in range(len(model_checkpoints)):
    ...
    with open(Path('weights', title), 'wb') as f:
    ...
    for i in range(1):
        ...
        satellites_gt_path = Path('SALAS_Rep', 'images', 'solar_validV2.json')
        ...
        satellites_path = Path('weights', title)
        ...
        #--------------------------------
        print("Length of Gt: " + str(len(iset_satellites_gt)))
        print("Length of pred: " + str(len(iset_satellites_pred)))
        print("GT size: " + str(gt.instances.image_size))
        print("Pred size: " + str(pred.instances.image_size))
        print(gt)
        print(pred)
        gtmasks = analyze.masks_to_rle(gt, gt.instances.image_size)
        predmasks = analyze.masks_to_rle(pred, pred.instances.image_size)
        newPredMask = []
        for i in range(len(gtmasks)):
            newPredMask.append(predmasks[i])
        print(len(gtmasks))
        print(len(newPredMask))
        detection_results_ = analyze.rle_instance_matcher(gtmasks, newPredMask, iou_thresh=0.5, size=gt.instances.image_size)
