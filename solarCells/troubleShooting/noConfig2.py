...
root = '../../'
ocean_images = root + '../SALAS_Rep/satellite_images/'
...
NUM_ITERATIONS = 20                              # The total number of training iterations
CHECKPOINT_NUM = 20                              # This is the number of iterations before a checkpoint is stored
...
OUTPUT_FILE = 'NO_CONFIG2/NO_CONFIG_OUTPUT-2.txt'           # This is the file that stores precision scores
...
for loop_num in range(NUM_MODELS):
    ...
    cfg.OUTPUT_DIR = str(Path('weights', TEMP_FOLDER))
    ...
    for cycle in range(len(model_checkpoints)):
        ...
        with open(Path('weights', TEMP_FOLDER, title), 'wb') as f:
        ...
        for i in range(1):
            ...
            satellites_gt_path = json_path_val
            ...
            satellites_path = Path('weights', TEMP_FOLDER, title)
    ...
    for file in range(len(pickle_folder)):
        temp = "weights/" + TEMP_FOLDER + "/" + pickle_folder[file]
        print("Deleting: " + temp)
        print(temp)
        os.remove(temp)
    print("Removing: " + "weights/" + TEMP_FOLDER  + "/" +"metrics.json")
    os.remove("weights/" + TEMP_FOLDER  + "/" +"metrics.json")
    print("Removing: " + "weights/" + TEMP_FOLDER  + "/" +"last_checkpoint")
    os.remove("weights/" + TEMP_FOLDER  + "/" + "last_checkpoint")
