...
def sort_and_clean(input_list):
    ...
    TODO: Switch from iqr to 3 standard deviations
    '''
    sort = []
    for i in input_list:
        sort.append(i)
    ...
    up_lim = mean + 3*std
    low_lim = mean - 3*std
    upper_index_cut_off = len(sort) - 1
    lower_index_cut_off = 0
    for i in range(len(sort)):
        if sort[i] > low_lim:
            lower_index_cut_off = i
        if sort[i] > up_lim:
            upper_index_cut_off = i
            break
    return sort[lower_index_cut_off:upper_index_cut_off]
