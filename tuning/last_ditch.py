...
cfg.OUTPUT_DIR = str(Path('DEMO_TEST3'))
...
for cycle in range(len(model_checkpoints)):
    ...
    file_name = 'SALAS_Replication_T.txt'
    ...
    if iteration_name == 'final':
        ...
        return_list = ["LD NA", "LD NA", 'LD NA', str(int(iteration_name)), str(sum(average_p[0])/len(average_p[0])), str(sum(average_r[0])/len(average_r[0]))]
