...
NUM_MODELS = 3                                      # This is the number of models that will be trained from scratch
OFFSET = 0                                          # This is used if trainings are split into A and B, if A, = 0, if B, = NUM_MODELS
TEMP_FOLDER = 'batch_temp21'                         # The file that model weights will be stored after training before being analyzed
OUTPUT_FILE = '../CPU_NO_CONFIG_Output-0.txt'       # This is the file that stores precision scores
...
FINAL_MODEL_FOLDER = '../../../../../../../ocean/projects/dmr200021p/sprice/variance/CPU_NO_CONFIG/trial1/'
#--------------------------------------------------------------
for loop_num in range(NUM_MODELS):
    ...
    for cycle in range(len(model_checkpoints)):
        ...
        if iteration_name == 'final':
            ...
            return_list = [loop_num+OFFSET, str(sum(average_p[0])/len(average_p[0])), str(sum(average_r[0])/len(average_r[0])), 'Shared']
